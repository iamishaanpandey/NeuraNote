...
# Initialize API Key
if getattr(sys, 'frozen', False):
    internal_key = os.path.join(BASE_DIR, "api_key.txt")
    if os.path.exists(internal_key) and not os.path.exists(API_KEY_FILE):
        shutil.copy(internal_key, API_KEY_FILE)
...
async def analyze_note(
    ...
    except: raise HTTPException(500, "API Key missing")
...
if __name__ == "__main__":
    ...
    api = WindowAPI()
    splash = """<!DOCTYPE html><html><body style="background:#020817;color:white;display:flex;flex-direction:column;justify-content:center;align-items:center;height:100vh;margin:0;font-family:sans-serif;"><h1>ST NeuraNote</h1><p>Loading Intelligence...</p></body></html>"""
    window = webview.create_window("ST NeuraNote", html=splash, width=1366, height=768, js_api=api, resizable=False, frameless=True)
    def transition():
        while True:
            try: socket.create_connection(('127.0.0.1', 8000), timeout=1); break
            except: time.sleep(0.5)
        window.load_url("http://127.0.0.1:8000")
        time.sleep(0.5)
    webview.start(transition)